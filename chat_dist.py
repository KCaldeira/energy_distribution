...
#%%
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
# Chat GTP insprired code

def global_energy_population_distribution(per_capita_energy, pop, pct_grid=None):
    """
    Computes global cumulative distributions of energy use by population and vice versa.

    Parameters:
        per_capita_energy: np.array (shape: num_countries x num_percentiles)
            Per capita energy consumption for each country across population percentiles.
        pop: np.array (shape: num_countries,)
            Total population of each country.
        pct_grid: np.array (optional, shape: num_percentiles,)
            Percentiles associated with the columns of per_capita_energy. Defaults to linspace(0, 1).

    Returns:
        pop_to_energy_fn: function
            Interpolation function mapping population fraction [0, 1] → cumulative energy fraction [0, 1]
        energy_to_pop_fn: function
            Interpolation function mapping per capita energy value → cumulative population fraction [0, 1]
        sorted_energy: np.array
            Sorted per capita energy values (for plotting or inspection)
        pop_percentiles: np.array
            Corresponding cumulative population fractions
        energy_percentiles: np.array
            Corresponding cumulative energy fractions
    """
    num_countries, num_bins = per_capita_energy.shape

    if pct_grid is None:
        pct_grid = np.linspace(0, 1, num_bins)

    # Compute population share for each percentile bin
    pop_bins = np.diff(np.append([0], pct_grid))  # size: num_bins
    pop_share = pop[:, None] * pop_bins  # shape: (num_countries, num_bins)

    # Flatten arrays
    flat_energy = per_capita_energy.flatten()
    flat_pop_share = pop_share.flatten()

    # Sort by energy use
    sort_idx = np.argsort(flat_energy)
    sorted_energy = flat_energy[sort_idx]
    sorted_pop_share = flat_pop_share[sort_idx]

    # Cumulative population and energy
    cum_pop = np.cumsum(sorted_pop_share)
    total_pop = cum_pop[-1]
    pop_percentiles = cum_pop / total_pop

    energy_per_group = sorted_energy * sorted_pop_share
    cum_energy = np.cumsum(energy_per_group)
    total_energy = cum_energy[-1]
    energy_percentiles = cum_energy / total_energy

    # Create interpolation functions
    pop_to_energy_fn = interp1d(
        pop_percentiles, energy_percentiles, bounds_error=False, fill_value=(0, 1)
    )

    energy_to_pop_fn = interp1d(
        sorted_energy, pop_percentiles, bounds_error=False,
        fill_value=(0, 1)  # Could also set to (0,0) and (1,1) if you prefer
    )

    return pop_to_energy_fn, energy_to_pop_fn, sorted_energy, pop_percentiles, energy_percentiles
